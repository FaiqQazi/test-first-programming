...
public class ExtractTest {
    ...
    public void testGetMentionedUsersSingleMention() {
        Tweet tweet = new Tweet(1, "user1", "Hello @user2", d1);
        ...
        assertTrue("expected user2 to be mentioned", mentionedUsers.contains("user2"));
    }

    @Test
    public void testGetMentionedUsersMultipleMentions() {
        Tweet tweet = new Tweet(1, "user1", "Hello @user2 and @User3", d1);
        Set<String> mentionedUsers = Extract.getMentionedUsers(Arrays.asList(tweet));
        assertEquals("expected two mentioned users", 2, mentionedUsers.size());
        assertTrue("expected user2 to be mentioned", mentionedUsers.contains("user2"));
        assertTrue("expected user3 to be mentioned", mentionedUsers.contains("user3"));
    }

    @Test
    public void testGetMentionedUsersDuplicateMentions() {
        Tweet tweet1 = new Tweet(1, "user1", "Hello @user2", d1);
        Tweet tweet2 = new Tweet(2, "user2", "@user2 is here too!", d2);
